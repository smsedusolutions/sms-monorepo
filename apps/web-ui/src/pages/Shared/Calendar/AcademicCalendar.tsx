...
export const AcademicCalendar: React.FC = () => {
    ...
    // Returns true if the event's end date (or start if no end) is strictly in the past
    const isPastEvent = (ev: CalendarEvent): boolean => {
        const refDate = ev.endDate || ev.startDate;
        if (!refDate) return false;
        const d = typeof refDate === 'string' ? parseISO(refDate) : new Date(refDate);
        const today = new Date();
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                                                            ...
                                                            />
                                                            {isAdmin && !ev.isAutoGenerated && !isPastEvent(ev) && (
                            ...
                            <AppDatePicker
                                ...
                                onChange={val => {
                                    ...
                                    setForm(f => ({
                                        ...f,
                                        startDate: val,
                                        // Auto-correct endDate if it is before the new startDate
